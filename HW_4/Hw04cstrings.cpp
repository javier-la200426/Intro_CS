/*
 * Javcstring.cpp
 * Purpose: To complete the is_equal function (Hw04)
...
bool jav_substring(char *haystack, char *needle); //Jav modified (delete later)
...
/*
  ...
  //Jav Prefix text 
    ...
    bool prefix = is_prefix(&word1[0], &word2[0]);
    if (prefix) {
      cout << "prefix" << endl;
    } else {
  ...
  */
    ...
    char haystack[] = "substri";
    char needle[] = "string";

    /*

    bool substring = jav_substring(&word1[0], &word2[0]);
    if (substring) {
      cout << "substring" << endl;
    } else {
   ...
   jav_substring(&haystack[0], &needle[0]);
 ...
 */
bool jav_substring(char *haystack, char *needle) { //char *find_substring(char *haystack, char *needle) //return nullptr;
  /* YOUR CODE GOES HERE. DELETE THE RETURN STATEMENT BELOW WHEN YOU'RE WORKING
   * ON THIS FUNCTION. */
  ...
  char arr[100]; //char arr[100] = {'\0'};
  char *new_string = &arr[0];
  char new_string_char = *(new_string);
  ...
  while (char1 != '\0') { 
        if ((char1 == char2)) {
          ...
          } else { //reset new temporary array to an 'empty' array
            for (int h=0; h<100; h++) { 
             arr[h] = '\0'; 
            }
          }
    ...
    if (equality) {
          ...
          cout << endl;
            ...
            cout << "Not substrings" << endl;
          }
    
    //just printing stuff to see new string and string
    /* new_string_char = *(new_string);
    for (int i = 0; i < 6 ; i++) {
       new_string_char = *(new_string + i);
       cout << new_string_char;
        }
        cout << endl;
        */
      //prints the same as above (the new string)
    for (int i = 0; i < 6 ; i++) {
       new_string_char = arr[i];
       cout << new_string_char;
        }
        cout << endl;
/*
  ...
  //go through hastack until the first character of needle equals haystack character

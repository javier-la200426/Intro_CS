...
int main(int argc, char *argv[]) {
  ...
  if (infile.fail()) {
    ...
    char haystack[] = "substring";
    char needle[] = "string";

    char *substring = find_substring(&haystack[0], &needle[0]);

    if (substring) {
    char value = *(substring); //this line causes segmentation fault wihtout if statement
...
/*
//just to print stuff. it does give the correc thing "string" in substring
char char1 = *(substring);
    int j = 0;
      while (char1 != '\0') {
        cout << char1;
        j++;

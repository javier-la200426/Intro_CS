/*
 * Javcstring.cpp
 * Purpose: To complete the is_equal function (Hw04)SUU
 ...
 */
 ...
 // }
  ...
  while (i < total_words) { //iterates over rows / words
    current_char = words[i][j];
    while (current_char != '\0') { //iterates over columns/character
      j++;
      current_char = words[i][j];
      cout << current_char; 
    }
...
char *find_substring(char *haystack, char *needle) { // //return nullptr;
  ...
  int i = 0;
  char1 = tolower(*(haystack)); //resetting character of haystack to the value at 0th index
  char2 = tolower(*(needle)); // resetting character of needle to 0th index. 
